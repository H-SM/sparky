...
def create_spark_session():
    ...
    spark = (SparkSession.builder
        ...
        .config("spark.driver.memory", "12g")  # Updated to 12g as in the new script
...
def load_and_save_to_hdfs(spark, file_path, hdfs_path, data_format="string"):
    """
    ...
    hdfs_path: HDFS path to save the data
    data_format: Format of node IDs ("long" or "string")
    """
    print("\n--- Loading Data to HDFS ---")
    
    # Define schema for edge dataset based on format
    if data_format.lower() == "long":
        schema = StructType([
...
def load_edge_dataset(spark, file_path, use_hdfs=True, data_format="string"):
    """
    ...
    spark: SparkSession object
    file_path: Path to the edge list file
    use_hdfs: Whether to use HDFS or local file system
    data_format: Format of node IDs ("long" or "string")
    """
    # Define schema for edge dataset based on format
    if data_format.lower() == "long":
    ...
    # Read edge list (from HDFS if available, else from local)
    file_source = file_path if not use_hdfs else f"hdfs://localhost:9000{file_path}"
    try:
        edges_df = spark.read.schema(schema).csv(file_source, sep=" ")
        print(f"Successfully loaded data from: {file_source}")
    except Exception as e:
        print(f"Error reading from {file_source}: {e}")
...
def pagerank_analysis_graphx(spark, graph_x, jvm, iterations=10, data_format="string"):
    """
    Perform PageRank analysis using native GraphX implementation
    
    Parameters:
    spark: SparkSession object
    graph_x: GraphX graph object
    jvm: Java VM object
    ...
    try:
        ...
        # Convert to DataFrame for easier analysis - schema depends on data format
        if data_format.lower() == "long":
            schema = StructType([
...
def community_detection(spark, graph, graph_x=None, jvm=None, data_format="string"):
    """
    ...
    if graph_x and jvm:
        try:
            ...
            # Convert to DataFrame - schema depends on data format
            if data_format.lower() == "long":
                schema = StructType([
                    StructField("id", LongType(), True),
...
def main():
    ...
    try:
        ...
        # File paths for edge lists
        gplus_edge_file = "gplus_combined.txt"
        
        # Allow selecting which dataset to analyze
        # Default to Google+ dataset as in the newer script
        selected_dataset = gplus_edge_file
        data_format = "long"  # Use StringType for Google+ dataset
        ...
        hdfs_file_path = load_and_save_to_hdfs(spark, selected_dataset, hdfs_path, data_format)
        
        # Load dataset (from HDFS if available)
        vertices, edges = load_edge_dataset(spark, selected_dataset, use_hdfs=False, data_format=data_format)
        
        # Create GraphFrame
        graph = GraphFrame(vertices, edges)
        
        # Initialize GraphX objects (will be None if GraphX is not properly configured)
        graph_x = None
        jvm = spark._jvm
        
        # Attempt to create GraphX graph (advanced feature, may not work in all environments)
        try:
            # Get RDDs from DataFrames
            vertices_rdd = vertices.rdd.map(lambda row: (row["id"], row["index"]))
            edges_rdd = edges.rdd.map(lambda row: (row["src"], row["dst"]))
            
            # Create GraphX graph
            if data_format.lower() == "long":
                # For numeric node IDs
                graph_x = jvm.org.apache.spark.graphx.Graph.apply(
                    vertices_rdd._jrdd.map(jvm.scala.Tuple2(spark._jvm.java.lang.Long, spark._jvm.java.lang.Long)),
                    edges_rdd._jrdd.map(jvm.org.apache.spark.graphx.Edge(spark._jvm.java.lang.Long, spark._jvm.java.lang.Long)),
                    spark._jvm.java.lang.Long.valueOf(0),
                    spark._jvm.scala.reflect.ClassTag.apply(spark._jvm.java.lang.Long),
                    spark._jvm.scala.reflect.ClassTag.apply(spark._jvm.java.lang.Long)
                )
            else:
                # For string node IDs
                graph_x = jvm.org.apache.spark.graphx.Graph.apply(
                    vertices_rdd._jrdd.map(jvm.scala.Tuple2(spark._jvm.java.lang.String, spark._jvm.java.lang.Long)),
                    edges_rdd._jrdd.map(jvm.org.apache.spark.graphx.Edge(spark._jvm.java.lang.String, spark._jvm.java.lang.String)),
                    spark._jvm.java.lang.String.valueOf(""),
                    spark._jvm.scala.reflect.ClassTag.apply(spark._jvm.java.lang.String),
                    spark._jvm.scala.reflect.ClassTag.apply(spark._jvm.java.lang.String)
                )
            
            print("Successfully created GraphX graph object")
        except Exception as e:
            print(f"Failed to create GraphX graph: {e}")
            print("GraphX-specific analyses will be skipped")
        ...
        # Run GraphX PageRank if GraphX graph is available
        if graph_x:
            pagerank_graphx = pagerank_analysis_graphx(spark, graph_x, jvm, data_format=data_format)
        
        # Community detection using both implementations
        community_components = community_detection(spark, graph, graph_x, jvm, data_format=data_format)
