...
def pagerank_analysis_optimized(spark, graph, iterations=10, checkpoint_interval=5):
    """
    Perform PageRank analysis with checkpointing for large graphs
    """
    print(f"\n--- PageRank Analysis (Iterations: {iterations}) ---")
    start_time = time.time()
    
    try:
        # Run PageRank with only maxIter (remove tol parameter)
        ...
        print(f"PageRank completed in {time.time() - start_time:.2f} seconds")
        return pr.vertices
    except Exception as e:
...
def batch_process_large_graph(spark, vertices_df, edges_df, max_partition_size=5000000):
    ...
    if total_edges > max_partition_size:
        ...
        pagerank_results = pagerank_analysis_optimized(spark, sample_graph, iterations=10)
        # pagerank_results = pagerank_analysis_optimized(spark, sample_graph, iterations=20)
        # pagerank_results = pagerank_analysis_optimized(spark, sample_graph, iterations=50)
        # pagerank_results = pagerank_analysis_optimized(spark, sample_graph, iterations=100)
        # community_components = community_detection_optimized(spark, sample_graph, max_iterations=5)
        ...
        # visualize_degree_distribution_optimized(spark, degrees)
        pagerank_results = pagerank_analysis_optimized(spark, graph, iterations=10)
        # pagerank_results = pagerank_analysis_optimized(spark, graph, iterations=20)
        # pagerank_results = pagerank_analysis_optimized(spark, graph, iterations=50)
        # pagerank_results = pagerank_analysis_optimized(spark, graph, iterations=100)
        # community_components = community_detection_optimized(spark, graph, max_iterations=5)
