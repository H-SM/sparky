...
# No schema needed as Parquet files contain schema information
taxi_df = spark.read.parquet("yellow_tripdata_2023-01.parquet")
...
avg_fare_by_borough.write.csv("results/avg_fare_by_borough", header=True)
summary_stats.write.csv("results/summary_stats")

# Drop the 'features' column before saving the predictions
predictions_without_features = predictions.drop("features")

# Save predictions to CSV without the 'features' column
predictions_without_features.write.csv("results/predictions", header=True)
